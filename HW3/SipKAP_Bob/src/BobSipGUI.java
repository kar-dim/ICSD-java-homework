...
public class BobSipGUI extends BobEncryptedConnection {
    ...
    public BobSipGUI(ObjectInputStream ois, ObjectOutputStream oos, SecretKey secret, IvParameterSpec iv, String token, Mac hmac) {
        ...
        frame = new JFrame("Bob VoIP Caller");
    ...
    private void setListeners() {
        ...
        register.addActionListener((ActionEvent e) -> {
            ...
            registerframe.setLocationRelativeTo(BobSipGUI.this.frame);
        });
        ...
        register_r.addActionListener((ActionEvent e) -> {
            ...
            try {
                ...
                Logger.getLogger(BobSipGUI.class.getName()).log(Level.SEVERE, null, ex);
        ...
        login.addActionListener((ActionEvent e) -> {
            ...
            loginframe.setLocationRelativeTo(BobSipGUI.this.frame);
        });
        ...
        //αν πατηθεί το "CALL" τότε στέλνουμε στον proxy το INVITE μας με τα αρχικά stream.
        //στη συνέχεια ο proxy θα συνδεθεί μαζί μας όποτε απαντήσει ο άλλος Proxy (δηλαδή όποτε απαντήσει με ΝΑΙ ή ΌΧΙ στην αποδοχή ο άλλος client)
                ...
                synchronized (this) {
                    Runnable client_wait = () -> {
                        try {
                            ...
                            SIPMessage invite = new SIPMessage(Sip.INVITE, logged_in.getContact(), "<sips:alice@proxyA.com>", this.token, this.HMAC_Sign(logged_in.getContact() + this.token));
                            ...
                            if (is_online.getMessage().equals("USER_ONLINE")) {
                                ssock_active = new ServerSocket(8004); //8002 port -> (active) Alice, 8004 (active) port -> Bob
                                ...
                                if (!ok.getType().equals(Sip.OK)) {
                                    updateScreen("ALICE REJECTED YOUR CALL");
                                } else {
                                    updateScreen("GOT \"OK\"");
                                    ...
                                    this.i2p_direct_output.writeObject(new SIPMessage(Sip.ACK, "<sips:bob@proxyB.com>", "<sips:alice@proxyA.com>", null, null));
                        ...
                        } catch (IOException | ClassNotFoundException | ConnectionNotSafeException ex) {
                            this.call.setEnabled(true);
                            Logger.getLogger(BobSipGUI.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void makeClientAvailable() {
        ...
        synchronized (this) {
            Runnable client_wait = () -> {
                while (true) {
                    try {
                        ssock_passive = new ServerSocket(8002); //8001 port -> Alice, 8002 port -> Bob
                        ...
                        if (mode.equals("false")) {
                            ...
                            //στέλνουμε RINGING
                            this.i2p_outputstream_passive.writeObject(encryptSip(new SIPMessage(Sip.RINGING, "<sips:bob@proxyB.com>", "<sips:alice@proxyA.com>", this.token, this.HMAC_Sign("<sips:bob@proxyB.com>" + this.token))));
                            ...
                            int reply = JOptionPane.showConfirmDialog(BobSipGUI.this.frame, prx_invite.getFrom() + "is calling! Accept?", "Call", JOptionPane.YES_NO_OPTION);
                            if (reply == JOptionPane.YES_OPTION) {
                                this.i2p_outputstream_passive.writeObject(encryptSip(new SIPMessage(Sip.OK, "<sips:bob@proxyB.com>", "<sips:alice@proxyA.com>", this.token, this.HMAC_Sign("<sips:bob@proxyB.com>" + this.token))));
                                this.i2p_outputstream_passive.flush();
                                ...
                                this.i2p_outputstream_passive.writeObject(encryptSip(new SIPMessage(Sip.UNAUTHORIZED, "<sips:bob@proxyB.com>", "<sips:alice@proxyA.com>", this.token, this.HMAC_Sign("<sips:bob@proxyB.com>" + this.token))));
                                this.i2p_outputstream_passive.flush();
